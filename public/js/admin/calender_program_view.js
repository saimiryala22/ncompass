...
function addsource_details_programs(zone,emp_id){
			...
			for( var k=0;k<ins1.length;k++){
				...
				if(k!=j){
					if(program_id==program_id1){
						alertify.alert("Program Selection should be Unique");
						return false;
					}
				}
			}
	...
	$("#source_table_programs").append("<tr id='subgrd"+sub_iteration+"'><td><input type='checkbox' id='chkbox"+sub_iteration+"' name='chkbox[]' value='"+sub_iteration+"'><input type='hidden' id='zone_id"+sub_iteration+"' name='zone_id[]' value='"+zone+"'><input type='hidden' id='employee_id"+sub_iteration+"' name='employee_id[]' value='"+emp_id+"'><input type='hidden' id='calendar_program_id"+sub_iteration+"' name='calendar_program_id[]' value=''></td><td><select name='category_id[]' id='category_id"+sub_iteration+"' onchange='open_programs("+sub_iteration+");' style='width:200px;'><option value=''>Select</option>"+cat_ids+"</select></td><td style='text-align:center'><select name='program_id[]' id='program_id"+sub_iteration+"' style='width:400px;'><option value=''>Select</option></select></td></tr>");
	...
	/* <td><select name='zone_id["+sub_iteration+"][]' id='zone_id"+sub_iteration+"' style='width:300px;'>"+zone_ids+"</select></td> */
}
...
function open_program_events(id,cal_id,cal_month_id,pro_id){
	...
	xmlhttp.open("GET",BASE_URL+"/admin/calender_program_events?pro_id="+id+"&cal_id="+cal_id+"&cal_month_id="+cal_month_id+"&program_id="+pro_id,true);
	xmlhttp.send();
...
function search_data(){
	...
	document.getElementById('work_div').innerHTML="loading..."
	xmlhttp.onreadystatechange=function(){
		if (xmlhttp.readyState==4 && xmlhttp.status==200){
			document.getElementById('work_div').innerHTML=xmlhttp.responseText;
		}
	}
	
	xmlhttp.open("GET",BASE_URL+"/admin/program_preparation_view_source?calendar_month_id="+calendar_month_id+"&calendar_id="+calendar_id+"&location_id="+location_id+"&employee_id="+employee_id,true);
	xmlhttp.send();
}