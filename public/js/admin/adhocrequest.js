...
function status_change(){
    ...
    if(status_typ=='S'){
        if(eve_status=='P'){
            ...
            alert("You cannot modify the event from Scheduled to planned , once the event starts");
            return false;
        }
    }//alert(evnt_cnt);
    ...
    if(evnt_cnt!='' && enrl_cnt>=evnt_cnt){
        if(eve_status!='F'){
            ...
            alert("Event Status can not be changed as Enrolment count exceeds Event Maximum count");
            return false;
        }
    }
	var maxcnt=document.getElementById('max_cou').value;
	if(maxcnt=='' && eve_status=='F'){
		alert("Event Status can not be changed to full when maximum count is empty");
...
$(document).ready(function() {
 ...
 function eval_enrolled(){
     alert("You can not perform any action after Enrollment");
     return false;
 }
                    ...
                    var attatchment2=document.getElementById('filename['+l+']').value;
                     ...
                     if(k!=j){
                        if(att1.trim()==att2.trim() && att_typ1.trim()==att_typ2.trim()){
                             alert("Attachment Name with same Type Already Exists");
                             return false;
                         }
                     }
                 }
				...
				for( var k=0;k<ins1.length;k++){
					...
					if(k!=j){
						if(item_type==item_type2 && item_resource==item_resource2){
							alert("Item type with same resource already exists");
...
function DelExpenses(){
	...
	if(flag==0){
		alert("Please select the Value to Delete");
		return false;
	}
...
function AddMore(){
       ...
       $("#fleupload_tab").append("<tr id='innertable"+sub_iteration+"'><td style='padding-left:20px;'><input type='hidden' name='attach_id[]' id='attach_id["+sub_iteration+"]' value=''><input type='checkbox' name='select_chk[]' id='select_chk_"+sub_iteration+"' value='"+sub_iteration+"' /></td><td><input type='hidden' name='eve_attach' id='eve_attach_"+sub_iteration+"' value=''></td><td><input class='validate[custom[file]]' type='file' name='fileupload[]' id='fileupload["+sub_iteration+"]' value='"+sub_iteration+"' style='width:175px;' /></td><!--<input type='hidden' id='hide_values["+sub_iteration+"]' name='hide_values[]' value='' />--></td></tr>");
       if(document.getElementById(hiddtab).value!=''){
           var ins=document.getElementById(hiddtab).value;
           document.getElementById(hiddtab).value=ins+","+sub_iteration;
       }
       ...
       for(var i=(arr1.length-1); i>=0; i--){
           ...
           if(document.getElementById(a).checked){
               ...
               var c="innertable"+b+"";
                ...
                if(attachmentid!=' '){
                    $.ajax({
                        url: BASE_URL+"/admin/delete_adhoc_attachments",
                        global: false,
                        type: "POST",
...
function cant_update_enroll(){
    alert("Can not update Enrollment when status is either Cancelled or Attended");
}
function cannot_enroll(){
    alert("You cannot enroll participants, because event has been end dated ");
}
function enrl_status_validates(){//alert(evnt_cnt);
	...
	if(sysdate<evnt_stdate){
		if(enrl_status=='ATT'){
			alert("Event status can not be changed to Attended before starting the event");
    ...
    if(evnt_cnt==''){
        if(enrl_status=='WAIT'){
            alert("Employee can not be waitlisted when there is no maximum count for the event");
            document.getElementById('enroll_status').value='REQ';
            return false;
        }
    }
        ...
        if(parseInt(enrl_cnt)>=parseInt(evnt_cnt)){
            if(enrl_status=='CON'){
                alert("You cannot update the enrollment status to Confirmed, if the enrollment count more than event maximum count");
...
function single_enroll(){//alert(evnt_cnt);
...
function bulk_enroll(){
    ...
    document.getElementById('enrollment_div').innerHTML="<label class='control-label bolder blue'><input type='radio' class='ace' name='bulkempdata' id='search_bulkemp' value='' checked='checked' onclick='change_bulk_type()'><span class='lbl middle'>&nbsp;&nbsp;Search for Employees</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n\
	<label class='control-label bolder blue'><input type='radio' class='ace' name='bulkempdata' id='upload_bulkemp' value='' onclick='change_bulk_type()'><span class='lbl middle'>&nbsp;&nbsp;Upload Employees</span></label>\n\
	<div id='search_bulkemp_data'><br>\n\
		<div class='row'>\n\
			<div class='col-xs-12 col-sm-3'>\n\
				Employee Number<br>\n\
				<select class='chosen-select' name='enrol_emp_no'  id='enrol_emp_no'>\n\
				<option value=''>Select</option>"+empdata+"</select>\n\
				<input type='hidden' name='progid' id='progid' value="+pro_id+">\n\
				<input type='hidden' name='evntid' id='evntid' value="+eve_id+">\n\
			...
			<div class='col-xs-12 col-sm-3'>\n\
				Employee Type<br>\n\
				<select class='col-xs-12 col-sm-12' name='enrol_employee_type' id='enrol_employee_type'><option value=''>Select</option>"+emp_type+"</select>\n\
			</div>\n\
			<div class='col-xs-12 col-sm-3'>\n\
				Organization<br>\n\
				<select class='col-xs-12 col-sm-12' name='enrol_department' id='enrol_department'><option value=''>Select</option>"+enrl_orgnames+"</select>\n\
			</div>\n\
		</div>\n\
		<div class='row'>\n\
			<div class='col-xs-12 col-sm-3'>\n\
				Location<br>\n\
				<select class='col-xs-12 col-sm-12' name='enrol_location' id='enrol_location'><option value=''>Select</option>"+locations+"</select>\n\
			</div>\n\
			<div class='col-xs-12 col-sm-3'>\n\
				Manager<br>\n\
				<select class='col-xs-12 col-sm-12' name='enrol_manager' id='enrol_manager'><option value=''>Select</option>"+managers+"</select>\n\
			</div>\n\
			<div class='col-xs-12 col-sm-3'>\n\
				Learner Group<br>\n\
				<select class='col-xs-12 col-sm-12' name='enrol_learner_group'  id='enrol_learner_group'><option value=''>Select</option>"+learner_groups+"</select>\n\
			</div>\n\
			<div class='col-xs-12 col-sm-3'><br>\n\
				<input type='button' class='btn btn-sm btn-default' name='search' id='search' value='Search' onClick='bulk_emp()'>\n\
			</div>\n\
		</div><br>\n\
	</div>\n\
	<div id='upload_empbulk_div' style='display:none;'><br>\n\
		<div class='row'>\n\
...
function bulk_emp(){//alert(eve_id);
        ...
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
			...
			if($response[0]==0){
				...
				if($response[0]!=""){
					...
					alert("Employee details does not found with this Employee Number");
					document.getElementById('emp_no').value='';
        ...
        for( var j=0;j<ins1.length;j++){
            ...
            if(attatchment!=''){
                for( var k=0;k<ins1.length;k++){
                     ...
                     if(k!=j){
                         if(att1.trim()==att2.trim() && att_typ1.trim()==att_typ2.trim()){
                             alert("Attachment with same type Already Exists");
                             return false;
                         }
                     }
                 }
...
function update_single_enroll(id){
    ...
    xmlhttp.open("GET",BASE_URL+"/admin/enroll_event_update?enrl_id="+id+"&prog_id="+prgid+"&evntid="+evntid+"&eve_cnt="+evnt_cnt,true);
    xmlhttp.send();
}
...
function cannot_restrict(){
	alert("Learner Access can not be done after Enrollment");
...
//    // To fetch Learner Positions from .php page
//    if(learner_pos!=''){
//        var lnr_pos=learner_pos.split(',');
//        var learner_positions='';
//        for(var i=0;i<lnr_pos.length;i++){
//            lnr_pos1=lnr_pos[i];
//            lnr_pos2=lnr_pos1.split('*');
//            if(learner_positions==''){
//                learner_positions="<option value='"+lnr_pos2[0]+"'>"+lnr_pos2[1]+"</option>";
//            }else{
//                learner_positions=learner_positions+"<option value='"+lnr_pos2[0]+"'>"+lnr_pos2[1]+"</option>";
//            }
//        }
//    }
//    else{
...
function resource_details(){
    ...
    xmlhttp.onreadystatechange=function()
    {
        if (xmlhttp.readyState==4 && xmlhttp.status==200)
        {
            var resp=xmlhttp.responseText.split('#*');
			if(resp[0]){
				$('#extra_info').html(resp[0]);
			}else{
				$('#extra_info').html(" ");
			}
			if(resp[1]){
				$('#extra_info1').html(resp[1]);
			}else{
				$('#extra_info1').html(" ");
			}
			if(resp[2]){
				$('#supplier').html(resp[2]);
			}else{
				$('#supplier').html("<option value=''>Select</option>");
			}
        }
    }
    xmlhttp.open("GET",BASE_URL+"/admin/resource_additionalinfo?res_def_id="+def_id,true);
    xmlhttp.send();
...
function cannot_evaluate(){
    alert("Evaluation can not be done after Enrollment");
}
function cannot_change(){
    alert("Cannot change to Public once the Enrollment is done");
...
function create_evaluation(){//alert(testmand);
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group'>\n\
					<input type='text' name='start_date_active' id='evalu_start_date' class='validate[custom[date2],future[#evalu_start_date]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' value='' onfocus='date_funct(this.id)'>\n\
...
function evaluate_validate(){
    ...
    var prgm_stdate=document.getElementById('prgm_strt_date').value;
    var prgm_enddate=document.getElementById('prgm_end_date').value;
	if(eval_stdate=='' && eval_enddate!=''){
		alert("End date can not be given with out giving the start date");
		return false;
	}
    ...
    if(delivery_method=='C'){
        if(test_type=='PRE'){
			if(stdate_eval=='' || eval_enddate==''){
				alert("Please give start date and end date for PRE Test");
				return false;
			}
            if(stdate_eval<stdate_enrol || stdate_eval>stdate_evnt){
                alert("Evaluation Start date should be between Enrolment Start Date and Event Start Date");
                return false;
            }
        }
    }
        ...
        if(test_type=='PRE' || test_type=='POST'){
            if(stdate_eval<stdate_enrol || stdate_eval>enddate_evnt){
                alert("Evaluation Start date should be between Enrolment Start Date and Event End Date");
                return false;
            }
            if(enddate_eval>enddate_evnt){
                alert("Evaluation End date should be Less than or Equal to Event End Date");
                return false;
            }
        }
    }
...
function forum_create(){
	...
	<div class='row' style='width: 100%;'>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group' style='margin-left: -13px;'>\n\
					<input type='text' name='start_date_active' id='forum_start_date' class='validate[custom[date2], future[#startdate], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' value='' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group' style='margin-left: -13px;'>\n\
					<input type='text' name='end_date_active' id='forum_end_date' value='' class='validate[custom[date2], future[#forum_start_date], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
...
function reference_create(){
	...
	<div class='row'>\n\
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group'>\n\
					<input type='text' class='validate[custom[date2], future[#startdate], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' name='start_date_active' id='start_date_active' value='' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group'>\n\
					<input type='text' class='validate[custom[date2], future[#start_date_active], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' name='end_date_active' id='end_date_active' value='' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
...
function reference_valid(){
	...
	if(stdate=='' && enddate!=''){
		alert("End date can not be given without giving the start date");
		return false;
	}
}
...
function chat_create(){
	...
	<div class='row' style='width: 100%;'>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group' style='margin-left: -13px;'>\n\
					<input type='text' name='start_date_active' id='start_date_active' value='' class='validate[custom[date2], future[#startdate], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy'  onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group' style='margin-left: -13px;'>\n\
					<input type='text' name='end_date_active' id='end_date_active' value='' class='validate[custom[date2],future[#start_date_active], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
...
function chattime_validate(){
	...
	if(stdate=='' && enddate!=''){
		alert("End date can not be given for Moderator without giving the start date");
		return false;
	}
	if(enddate!='' && Number(enddate)<Number(stdate)){
		alert("End date for Moderator should be greater than the start date");
		return false;
	}
    if(stdate!='' && enddate!=''){
        if(stdate==enddate){
            if(endtime<=sttime){
                alert("End time can not be less than start time for the same day");
                return false;
            }
        }
    }else{
		if(stdate=='' && enddate==''){
			if(sttime!='' || endtime!=''){
				if(sttime!='0:00:00' || endtime!='0:00:00'){
					alert("Time can not be given without giving both the dates");
					return false;
				}
			}
		}
...
function Topic_create(){
	...
	<div class='row' style='width: 100%;'>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group' style='margin-left: -13px;'>\n\
					<input type='text' name='topic_stdate' id='topic_stdate' value='' class='validate[custom[date2], future[#startdate], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
		...
		<div class='col-xs-12 col-sm-3'>\n\
			...
			<div class='input-large'>\n\
				<div class='input-group' style='margin-left: -13px;'>\n\
					<input type='text' name='topic_enddate' id='topic_enddate' value='' class='validate[custom[date2], future[#topic_stdate], past[#enddate]] datepicker input-large date-picker' data-date-format='dd-mm-yyyy' placeholder='dd-mm-yyyy' onfocus='date_funct(this.id)'>\n\
					<span class='input-group-addon'>\n\
						<i class='ace-icon fa fa-calendar'></i>\n\
					</span>\n\
				</div>\n\
...
function check_overlap_emp(){
	...
	if(evstdate==''){
		alert("End date can not be given without start date");
		return false;
	}
	if(evendate!='' && evstdate!=''){
		...
		xmlhttp.onreadystatechange=function()
		{
			if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				...
				if(delivery_method=='C'){
					if(spli[0]==1){
						alert("Employees Enrolled for this event has also been enrolled for other event on the same dates");
						document.getElementById('enddate').value=evendate_hidden;
						//location.reload();
					}
				}
				if(spli[2]==2){
					alert("End dates of "+spli[1]+" are greater than Event end date");
					document.getElementById('enddate').value=evendate_hidden;
					//alert("Employee has under gone pretest for this event, so date can not be pre ponded");
					//location.reload();
				}
			}
		}
