...
$filename=$emp_info['name'].'-'.$posdetails['position_name'].'-'.'Self Assessment Report';
...
<span style="font-size:20px;color:#fff;">'.$posdetails['position_name'].'<br><br><br><br></span><span style="font-size:12px;color:#fff;">Incumbent<br></span><span style="font-size:20px;color:#fff;">'.$emp_info['name'].'</span>';
...
$para2="The report is the self-assessment and development report of ".trim($emp_info['name']).", ".trim($posdetails['position_name']);
$tbl = <<<EOD
	...
	<table style="width:100%" align="left" cellspacing="0" border="0.5px" bordercolor="#5b9bd5" cellpadding="5" style="font-family: Helvetica; color:#646464;font-size:11px;line-height: 1.5; border:0.5px solid #9999;">
		<tbody>
			<tr bgcolor="#deeaf6">
				...
				<td class="normal"  style="width:20%"><b>Grade</b></td>
...
foreach($ass_comp as $key=>$ass_comps){$para="";
	$path = BASE_URL.'/public/reports/graphs/self/'.$_REQUEST['ass_id'].'/'.$ass_comps['category_id'].'_'.$_REQUEST['emp_id'].'_'.$_REQUEST['pos_id'].'_radar.svg';
		...
		foreach($ass_comp_info as $result){
			...
			<table>
				...
				$para.='</tr>
						...
						<span style="color:#5b9bd5;"><i>Required Level</i>: '.$result['req_scale_name'].'</span><br>
						<span style="color:#72d65e;"><i>Assessed Level</i>: '.$result['final_scaled_name'].'</span>
...
$pdf->setPageMark();
		...
		<td width="40%" align="left">
			<span style="color:#5b9bd5;"><i>RL : Required Level</i></span><br>
			<span style="color:#72d65e;"><i>AL : Assessed Level</i></span><br><br>';
	...
	<table style="width:100%" align="left" cellspacing="0" border="0.5px" bordercolor="#5b9bd5" cellpadding="5" style="font-family: Helvetica; color:#646464;font-size:11px;line-height: 1.5; border:0.5px solid #646464;">
		...
		foreach($ass_comp_info as $result){
			...
			$para.='<tr bgcolor="'.$bgcolor.'">
				<td style="width:25%">'.$result['comp_def_name'].'</td>
				<td style="width:15%">'.$result['comp_cri_name'].'</td>
				<td style="width:21%">'.$result['req_scale_name'].'</td>
				<td style="width:21%">'.$result['final_scaled_name'].'</td>
				<td style="width:18%">'.$vec.'</td>
			</tr>';
		}
	...
	if(count($competencies_name)>0){
		foreach($competencies_name as $key=>$competencies_names){
			if(!empty($competencies_names['strengths_desc'])){
				$para.='<b>'.$competencies_names['comp_def_name'].'</b><br>
				'.$competencies_names['strengths_desc'].'<br><br>';
			}
		}
	}
	else{
...
// set bacground image
$img_file = K_PATH_IMAGES.'position-bcoverpage.jpg';
...
$filename=$emp_info['name'].'_'.$posdetails['position_name'].'_'.'Self Assessment Report';
